         ...
         (cond ((eqv? (car expr) 'asub)
                (let ((aray (hash-ref *array-table* (cadr expr))))
                  (vector-ref
                   aray
                   (inexact->exact
                    (interpret-expression (caddr expr))))))
               (else
        ...
        (else
         (let ((header (car stmt)))
           (cond ((eqv? header 'goto) (cdr stmt))
                 ((eqv? header 'print)
                  (begin
                    (print-exprs (map interpret-expression (cdr stmt)))
                    '()))
                 ((eqv? header 'let)
                  (begin
          ...
          (let ((label-opt (interpret-statement stmt)))
            (begin
              (if (null? label-opt)
                  ...
                  (let ((program-counter
                         (hash-ref *label-table* (car label-opt) NAN)))
                    (begin
...
(define (write-program-by-line filename program)
  (let ((program-nolino (remove-lino program)))
    (begin
      ...
      (with-handlers
       ((exn:fail? (lambda (e) (printf "exception occurs: ~s\n" e))))
